...
<body>
    ...
    <main class="contenedor">
        ...
        <div class="grid">
            ...
            $Resultado=mysqli_query($db, 'SELECT *FROM `productos` LIMIT 0 , 30 ');
            while($row = mysqli_fetch_array($Resultado))
            {
            echo'<div class="producto">';
            echo        '<img class="producto__imagen" src="imgTiendaBD/'.$row['id'].'.jpg" alt="imagen camisa">';
            echo        '<div class="producto__informacion">';
            echo            '<p class="producto__des" style="text-align: center;">'.$row['descripcion'].'</p>';
            echo            '<p class="producto__nombre">'.$row['nombre'].'</p>';
            echo            '<div style="display: grid;
            grid-template-columns: repeat(2, 1fr);">';
            echo            '<p class="producto__precio">$'.$row['precio'].'</p>';
            echo            '<a class="producto__carrito" style="text-align: center;" href="productoBD.php?id='.$row['id'].'"><svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-shopping-cart-plus" width="52" height="52" viewBox="0 0 24 24" stroke-width="1.5" stroke="#d94f5c" fill="none" stroke-linecap="round" stroke-linejoin="round">
            <path stroke="none" d="M0 0h24v24H0z" fill="none"/>
    ...
    <div>
        <?php   
            ...
            if(file_exists($nombreArchivo)){
                echo '<a href="leerJSON.php"><h2>ver pedido...</h2></a>';
            }
        ?>
        </div>
    </main>
