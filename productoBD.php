...
if (!isset($_SESSION['idProfesor'])){
  ...
  include ("includes/acumularBD.php");
  }
 ?>
<body>
    ...
    <main class="contenedor">
        ...
        $conexion = mysqli_connect('127.0.0.1','tkdmaster','46d0596e3','tkdmaster')
        or die ("Fallo en el establecimiento de la conexion");

        $Resultado = mysqli_query($conexion, "SELECT * FROM `productos` WHERE `id` = '".$id."';");
        while($row = mysqli_fetch_array($Resultado))
        {
            echo '<div class="mostrar-producto">';
            echo '<img style="width: 40%" src="imgTiendaBD/'.$row['id'].'.jpg" class="compra">';
            echo '<form  action="acumularBD.php" method="post" name="Agregarcarrito">';
            echo '<div>';
            echo '<input type="text" name="nombre" value="'.$row['nombre'].'" readonly="readonly">';
            echo '</div>';
            echo '<div>';
            echo '<input type="number" name="precio" value="'.$row['precio'].'" readonly="readonly">MXM';
            echo '</div>';
            echo '<div>';
            echo '<input type="number" name="cantidad" size="2" max="10" min="1">';
            echo '</div>';
            echo '<div>';
            echo '<input type="submit" name="btnagregar" value="Agregar" id="btnagregar">';
            echo '</div>';
            echo '</form>';
            echo '</div>';
            }
